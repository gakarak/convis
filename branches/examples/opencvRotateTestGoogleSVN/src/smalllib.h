...
void drawArrow(IplImage* img, CvPoint2D32f p0, CvPoint2D32f p1 , CvScalar color, int thick=1, double zoom=1.0);

CvPoint2D32f getOrtoVec(CvPoint2D32f p);

inline double getLength(CvPoint2D32f p) {
	return sqrt(p.x*p.x + p.y*p.y);
}

inline CvPoint2D32f getDp(CvPoint2D32f p1, CvPoint2D32f p0) {
	return cvPoint2D32f(p1.x-p0.x, p1.y-p0.y);
}

inline CvPoint2D32f getSum(CvPoint2D32f p0, CvPoint2D32f add) {
	return cvPoint2D32f(p0.x + add.x, p0.y + add.y);
}

inline CvPoint2D32f getDel(CvPoint2D32f p0, CvPoint2D32f del) {
	return cvPoint2D32f(p0.x - del.x, p0.y - del.y);
}

inline CvPoint2D32f getDiv(CvPoint2D32f p, double num) {
	return cvPoint2D32f(p.x/num, p.y/num);
}

inline CvPoint2D32f getMul(CvPoint2D32f p, double num) {
	return cvPoint2D32f(p.x*num, p.y*num);
}
