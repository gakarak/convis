...
% Last Modified by GUIDE v2.5 20-Nov-2008 19:36:55
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.pushbutton12,'String','Start');
stop(handles.ttimer);
...
% handles    structure with handles and user data (see GUIDATA)
cnt = get(hObject,'String');
val = cnt{get(hObject,'Value')};

r1 = regexp(val,'_','split');
r2 = regexp(r1{2},'x','split');
axes(handles.axesXak);
handles.vid_res = [str2num(r2{1}), str2num(r2{2})];
...
function fcn_acquire_align(obj,event)
    hobj = get(obj,'UserData');
    handles = guidata(hobj);
% %     frm = peekdata(obj,1);
    frm = getdata(obj,1);
...
% %     disp(handles.scale);
    ...
    set(handles.axes_img,'CData',imadjust(rgb2gray(frm)));
% %     handles.old_frm = frm;
...
% %     frm(:,:,3) = adapthisteq(frm(:,:,3));
    ...
    dfrm = frm - handles.old_frm;

% %     bigfrm = handles.old_frm(handles.vid_bf_siz(2):handles.vid_bf_siz(2)+handles.vid_bf_siz(4),...
% %         handles.vid_bf_siz(1):handles.vid_bf_siz(1)+handles.vid_bf_siz(3),3);
% %     templ  = frm(handles.vid_sf_siz(2):handles.vid_sf_siz(2)+handles.vid_sf_siz(4),...
% %         handles.vid_sf_siz(1):handles.vid_sf_siz(1)+handles.vid_sf_siz(3),3);
    bigfrm = rgb2gray(handles.old_frm(handles.vid_bf_siz(2):handles.vid_bf_siz(2)+handles.vid_bf_siz(4),...
        handles.vid_bf_siz(1):handles.vid_bf_siz(1)+handles.vid_bf_siz(3),:));
    templ  = rgb2gray(frm(handles.vid_sf_siz(2):handles.vid_sf_siz(2)+handles.vid_sf_siz(4),...
        handles.vid_sf_siz(1):handles.vid_sf_siz(1)+handles.vid_sf_siz(3),:));

    cc = normxcorr2(templ,bigfrm);
    [max_cc, imax] = max(abs(cc(:)));
    [ypeak, xpeak] = ind2sub(size(cc),imax(1));
    set(handles.aim_rect,'Position',[handles.vid_bf_siz(1)+ xpeak, handles.vid_bf_siz(2)+ypeak, 3, 3]);
    
    dpx = ceil(xpeak - 2 - (handles.vid_bf_siz(3) + handles.vid_sf_siz(3))/2);
    dpy = ceil(ypeak - 2 - (handles.vid_bf_siz(4) + handles.vid_sf_siz(4))/2);
    
    
    if get(handles.checkVxVyPlot,'Value')
        handles.line_vx_data = circshift(handles.line_vx_data,1);
        handles.line_vx_data(1) = 0.001 * handles.scale * dpx;
        set(handles.line_vx,'YData',handles.line_vx_data);
        handles.line_vy_data = circshift(handles.line_vy_data,1);
        handles.line_vy_data(1) = 0.001 * handles.scale * dpy;
        set(handles.line_vy,'YData',handles.line_vy_data);
        if get(handles.checkXY,'Value')
            set(handles.line_vxvy,'XData',handles.line_vx_data,'YData',handles.line_vy_data);
        end
    end
    
    if get(handles.checkDirection,'Value')
        ll = handles.vv_arrow_length*sqrt(dpx^2 + dpy^2);
        handles.vv_arrow_xdata(2) = ll*dpx;
        handles.vv_arrow_ydata(2) = ll*dpy;
        set(handles.vv_arrow,'XData',handles.vv_arrow_xdata,'YData',handles.vv_arrow_ydata);
    end

    dt = toc;
    tic;
    if get(handles.checkVxVy,'Value')
        vx = 0.001 * handles.scale * dpx / dt;
        vy = 0.001 * handles.scale * dpy / dt;
        handles.vxvy3 = circshift(handles.vxvy3,1);
        handles.vxvy3(1,1) = vx;
        handles.vxvy3(1,2) = vy;
        set(handles.textVx1,'String',[num2str(vx,'%10.1f'),' m/s']);
        set(handles.textVx3,'String',[num2str(sum(handles.vxvy3(:,1)),'%10.1f'),' m/s']);
        set(handles.textVy1,'String',[num2str(vy,'%10.1f'),' m/s']);
        set(handles.textVy3,'String',[num2str(sum(handles.vxvy3(:,2)),'%10.1f'),' m/s']);
    end
    
    if handles.is_write_data
        handles.arr_data(handles.arr_data_counter,1) = now;
        handles.arr_data(handles.arr_data_counter,2) = 0.001 * handles.scale * dpx / dt;
        handles.arr_data(handles.arr_data_counter,3) = 0.001 * handles.scale * dpy / dt;
        handles.arr_data_counter = handles.arr_data_counter + 1;
    end
    
    if get(handles.checkPath,'Value')
        handles.xy_arr = circshift(handles.xy_arr,-1);
        handles.xy_arr(end,1) = handles.xy_arr(end-1,1) + dpx;
        handles.xy_arr(end,2) = handles.xy_arr(end-1,2) + dpy;
        set(handles.xy_plot,'XData',handles.xy_arr(:,1),'YData',handles.xy_arr(:,2));
    end
...
% %     oo = getselectedsource(handles.vid);
% %     set(oo,'FrameRate',10);
    %%%
    set(handles.vid,'StartFcn',@fcn_start);
    if get(handles.checkAlign,'Value')
        set(handles.vid,'FramesAcquiredFcn',@fcn_acquire_align);
    else
...
%%%% END
    ...
    handles.old_frm = getsnapshot(handles.vid);
    handles.aim_rect = rectangle('Position',[1, 1, 3, 3],'EdgeColor','blue');
% %     pause(1.5);

    tic;
    handles.vxvy3=zeros(3,2);
    handles.ttt = 0;
    start(handles.vid);
    guidata(hObject, handles);
end
...
handles.scale=1;
handles.is_write_data = 0;
handles.arr_data_counter = 1;
handles.arr_data = zeros(10000,3);
guidata(hObject, handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
set(hObject,'XGrid','on','YGrid','on');
handles.line_siz = 100;
handles.axes_line = hObject;
handles.line_xx_data = [1:1:100];
handles.line_vx_data = zeros(handles.line_siz,1);
handles.line_vx = line(handles.line_xx_data,handles.line_vx_data,'Color','red');
handles.line_vy_data = zeros(handles.line_siz,1);
handles.line_vy = line(handles.line_xx_data,handles.line_vy_data,'Color','blue');
guidata(hObject,handles);
...
handles.vv_arrow_xdata = [0 0.6]
handles.vv_arrow_ydata = [0 0.6];
handles.vv_arrow_length = sqrt(2*(0.6));
handles.vv_arrow = line(handles.vv_arrow_xdata,handles.vv_arrow_ydata,'LineWidth',2);
guidata(hObject, handles);
% Hint: place code in OpeningFcn to populate axes2


% --- Executes on button press in checkDirection.
function checkDirection_Callback(hObject, eventdata, handles)
% hObject    handle to checkDirection (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
get(hObject,'Value')
% Hint: get(hObject,'Value') returns toggle state of checkDirection


% --- Executes on button press in checkVxVyPlot.
function checkVxVyPlot_Callback(hObject, eventdata, handles)
% hObject    handle to checkVxVyPlot (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkVxVyPlot


% --- Executes on button press in checkVxVy.
function checkVxVy_Callback(hObject, eventdata, handles)
% hObject    handle to checkVxVy (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkVxVy
...
% handles    empty - handles not created until after all CreateFcns called
set(hObject,'XGrid','on','YGrid','on');
handles.line_vxvy = line(zeros(100,1),zeros(100,1));
guidata(hObject, handles);
% Hint: place code in OpeningFcn to populate axes4


% --- Executes on button press in checkXY.
function checkXY_Callback(hObject, eventdata, handles)
% hObject    handle to checkXY (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkXY
...
% handles    empty - handles not created until after all CreateFcns called
set(hObject,'XGrid','on','YGrid','on');
handles.siz_xy_arr = 100;
handles.xy_arr = zeros(handles.siz_xy_arr,2);
handles.xy_plot = plot(handles.xy_arr(:,1),handles.xy_arr(:,2));
guidata(hObject, handles);
% Hint: place code in OpeningFcn to populate axesXY


% --- Executes on button press in checkPath.
function checkPath_Callback(hObject, eventdata, handles)
% hObject    handle to checkPath (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkPath


% --- Executes during object creation, after setting all properties.
function checkXY_CreateFcn(hObject, eventdata, handles)
% hObject    handle to checkXY (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in checkAlign.
function checkAlign_Callback(hObject, eventdata, handles)
% hObject    handle to checkAlign (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if get(hObject,'Value')
    set(handles.editScaleX,'Enable','On');
    set(handles.editScaleY,'Enable','On');
else
    set(handles.editScaleX,'Enable','Off');
    set(handles.editScaleY,'Enable','Off');
    try
        XX=str2num(get(handles.editScaleX,'String'));
        handles.scale=XX/handles.vid_bf_width;
    catch Err1
        
    end
    try
        XX=str2num(get(handles.editScaleY,'String'));
        handles.scale=XX/handles.vid_bf_width;
    catch Err2
        handles.scale=1;
    end
...
guidata(hObject,handles);
% Hint: get(hObject,'Value') returns toggle state of checkAlign
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if handles.is_write_data == 1
    handles.is_write_data = 0;
    dlmwrite([datestr(now,30),'.txt'],handles.arr_data(1:handles.arr_data_counter,:),'delimiter',' ','precision',30);
    set(hObject,'String','Start');
else
    handles.is_write_data = 1;
    handles.arr_data = zeros(10000,3);
    handles.arr_data_counter = 1;
